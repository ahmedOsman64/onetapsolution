...
const AdminTopbar = ({ toggleSidebar }) => {
    ...
    return (
        <header className="sticky top-0 z-30 bg-black/60 backdrop-blur-md border-b border-white/5 h-16 flex items-center justify-between px-6">
            <div className="flex items-center gap-4">
                ...
                <div className="relative hidden sm:block">
                    ...
                    <input 
                        ...
                        className="bg-white/5 border border-white/10 rounded-full py-1.5 pl-10 pr-4 text-sm text-white focus:outline-none focus:border-[#04C244]/50 w-64 transition-all"
                    />
                </div>
            </div>
            ...
            <div className="flex items-center gap-4">
                ...
                <button className="relative p-2 rounded-lg hover:bg-white/5 text-slate-400 transition-colors">
                    <Bell size={20} />
                    <span className="absolute top-2 right-2 w-2 h-2 bg-[#04C244] rounded-full border-2 border-black"></span>
                </button>
                        ...
                        {showProfileMenu && (
                            <>
                                ...
                                <motion.div 
                                    ...
                                    className="absolute right-0 mt-2 w-48 bg-[#111318] border border-white/5 rounded-2xl shadow-2xl z-20 py-2 overflow-hidden"
                                >
                                    <button className="flex items-center gap-3 w-full px-4 py-2.5 text-sm text-slate-400 hover:bg-white/5 hover:text-white transition-all">
                                        <User size={16} />
                                    ...
                                    <button 
                                        onClick={logout}
                                        className="flex items-center gap-3 w-full px-4 py-2.5 text-sm text-red-400 hover:bg-red-400/10 transition-all"
                                    >
                                        <LogOut size={16} />
                                        <span>Log Out</span>
                                    </button>
