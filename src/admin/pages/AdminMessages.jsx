    ...
    return (
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                            ...
                            <motion.div 
                                ...
                                className={`
                                    ...
                                    ${selectedMessage?.id === msg.id 
                                        ...
                                        : msg.unread 
                                            ? 'bg-[#0A0C10] border-[#04C244]/20 hover:border-[#04C244]/40' 
                                            : 'bg-[#0A0C10] border-white/5 hover:border-white/10'
                                    }
                                `}
                            >
                                <div className="flex items-start justify-between mb-2">
                                    <h4 className={`text-sm font-bold ${msg.unread ? 'text-white' : 'text-slate-400'}`}>
                                        {msg.name}
                                    </h4>
                                    <span className="text-[10px] text-slate-500 font-bold uppercase">{msg.date}</span>
                                </div>
                                <p className={`text-xs font-medium truncate mb-2 ${msg.unread ? 'text-slate-300' : 'text-slate-500'}`}>
                                    {msg.subject}
                                </p>
                ...
                {/* Message View */}
                <div className={`lg:col-span-7 bg-[#0A0C10] border border-white/5 rounded-[40px] p-8 min-h-[500px] flex flex-col ${!selectedMessage ? 'hidden lg:flex items-center justify-center' : ''}`}>
                    <AnimatePresence mode="wait">
                        {selectedMessage ? (
                            ...
                            >
                                <div className="flex items-center justify-between mb-10 pb-8 border-b border-white/5">
                                    <div className="flex items-center gap-4">
                                        <button 
                                            ...
                                            className="p-2 bg-white/5 rounded-xl text-slate-500 hover:text-white lg:hidden"
                                        >
                                            <X size={20} />
                                        </button>
                                ...
                                <div className="flex-1 bg-white/2 border border-white/5 rounded-3xl p-8 mb-8 text-slate-300 text-sm leading-relaxed whitespace-pre-wrap">
                                ...
                                <div className="flex items-center gap-4">
                                    ...
                                    <button className="px-6 py-4 bg-white/5 border border-white/10 rounded-2xl text-sm font-bold text-slate-400 hover:text-white transition-all">
                                        Archive
                                    </button>
                                </div>
                            </motion.div>
                            ...
                            <div className="text-center space-y-4 max-w-xs mx-auto">
                                ...
                                <h3 className="text-lg font-bold text-slate-300">Select a Message</h3>
